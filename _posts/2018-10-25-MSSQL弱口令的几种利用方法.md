...
### \#0x00 写在前面 
2015年8月的时候对国内最大的某二手房地产公司做过一次安全测试，提交了wooyun，链接就不贴出来了。漏洞是由一个MSSQL注入引发，导致提权，最后完成内网漫游。最近又遇到了类似的情况，借这个机会记录一下MSSQL高权限漏洞的利用过程。  
...
图 net user admin 123 /add

图 net user

### \#0x03 提权Guest账户（账户克隆）
1、获取administrator账号的加密密码  
```
...
一、利用.NET环境，使用C#完成文件下载（下载文件mi.exe到目标机器） 
