...
![Mechanics of User Authentication]() 
{: refdef}
...
SAM database存储位置和获取方法： 
SAM database存储在注册表里， 路径为：HKEY_LOCAL_MACHINE\SAM，导出方法：reg.exe save hklm\sam c:\sam.hive  
...
存储位置和获取方法： 
LSA Secrets存储在注册表里， 路径为：HKEY_LOCAL_MACHINE/Security/Policy/Secrets，导出方法：reg.exe save hklm\security c:\security.hive  
...
存储位置和获取方法： 
从上面的介绍里可以看到Cached Domain Credentials存储路径与LSA Secrets一致，路径为：HKEY_LOCAL_MACHINE\HKLM\Security\Cache，导出方法：reg.exe save hklm\security c:\security.hive  
